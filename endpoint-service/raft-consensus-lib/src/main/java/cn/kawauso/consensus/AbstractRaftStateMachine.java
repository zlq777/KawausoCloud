...
/**
 * {@link AbstractRaftStateMachine}实现了{@link RaftStateMachine}中的主要逻辑，是整个状态机的核心。
 * 对于数据读写层相关基础设施方法，{@link AbstractRaftStateMachine}并不关心它们是怎么实现的。
 *
...
public abstract class AbstractRaftStateMachine implements RaftStateMachine {
    ...
    public AbstractRaftStateMachine(int index,
                                    int tickValue,
                                    int sendInterval,
                                    int minElectTimeout,
                                    int maxElectTimeout,
